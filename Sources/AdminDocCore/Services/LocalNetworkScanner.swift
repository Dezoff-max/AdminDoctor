...
public final class LocalNetworkScanner: @unchecked Sendable {
    ...
    private func warmARPTable(addresses: [String]) {
        ...
        let queue = DispatchQueue(label: "dev.admindoc.local-network-scan", attributes: .concurrent)
    ...
    private func resolveHostnames(from devices: [LocalNetworkDevice]) -> [String: String] {
        ...
        let queue = DispatchQueue(label: "dev.admindoc.local-network-name-resolution", attributes: .concurrent)
        let group = DispatchGroup()
